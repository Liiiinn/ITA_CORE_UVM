# ITA_CORE_UVM/tb Exercise Skeleton

`tb` is the learning version of the ITA/MHA8 UVM testbench. It should compile and start UVM, but important behavior is intentionally left as `TODO Stage N` exercises. Use `tb` as the reference implementation and do not edit it while working in `tb`.

## Ground Rules

- Keep the DUT target as `ita_mha8`; learn through head0 first.
- Preserve names and package style from `tb` where practical.
- Do not add `tb/seq`, `tb/scb`, or `tb/log` to the active filelist until their stage.
...
7. Logger
   - Files: env connect phase and a logger component restored or created under `tb/log`.
...
```powershell
Select-String -Path ITA_CORE_UVM\tb\**\*.sv,ITA_CORE_UVM\tb\**\*.svh,ITA_CORE_UVM\tb\AGENTS.md -Pattern "TODO Stage"
```

Check that complex packages are not active:

```powershell
Select-String -Path ITA_CORE_UVM\sim\filelist.f,ITA_CORE_UVM\tb\env\*.sv,ITA_CORE_UVM\tb\tests\*.sv -Pattern "ita_mha8_scb_pkg|ita_mha8_seq_pkg|ref_model|scoreboard|transaction_logger"
```
